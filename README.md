...
#### OpenCL on Raspberry Pi 5 (Vulkan GPU via `clvk`)
...
- `RASPTORCH_OPENCL_DEVICE`: substring match against device name/vendor
- `RASPTORCH_OPENCL_TILE`: matmul tiling (default `16`); the backend will auto-reduce if the device can’t support it

Example (force clvk if multiple OpenCL platforms exist):
```bash
RASPTORCH_OPENCL_PREFER=clvk uv run rasptorch backend connect opencl --strict
```
